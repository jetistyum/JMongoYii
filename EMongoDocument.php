...
class EMongoDocument extends EMongoModel
{
    ...
    {
        $this->trace(__FUNCTION__);
        $newcriteria = new EMongoCriteria();
        $newcriteria->setCondition($query);
        $newcriteria->setProject($project);
        ...
        foreach ($this->getSafeNonVirtualAttributeNames() as $attribute) {
            ...
            if ($value !== null && $value !== '') {
                if ((is_array($value) && count($value)) || is_object($value) || is_bool($value) || is_int($value)) {
